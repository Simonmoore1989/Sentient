...
export default function VendorSetup() {
  ...
  function getCombinedLink(name?: string, role?: string) {
    ...
    if (currentShutdownId) url += `&shutdownId=${currentShutdownId}`;
          ...
          <div style={{ position: 'relative' }}>
            ...
            {menuOpen && (
              <div onClick={e => e.stopPropagation()} style={{ position: 'fixed', top: 60, right: 20, background: th.surface, border: `1px solid ${th.border}`, borderRadius: 8, padding: 6, minWidth: 200, zIndex: 99999, boxShadow: '0 16px 48px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', gap: 2 }}>
                ...
                <div className="menu-item menu-item-danger" onClick={async () => { await supabase.auth.signOut(); router.push('/login'); }} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 5, fontFamily: "'Syne', sans-serif", fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#E05A5A', cursor: 'pointer' }}>
